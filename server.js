...
app.post('/lobbies', async (req, res) => {
  const { idLobbies, durationOfsesion, place } = req.body; 
  ...
  const firestore = new FirestoreService("lobbies");
 
  try {
    await firestore.PostDocument(idLobbies, data);
    res.status(200).send("User registered.");
  } catch (err) {
    console.error("noSQL error:", err);-
    res.status(500).send("Error registering user.");
...
app.post('/reportes', async (req, res) => {
  const { idReportes, razonReporte} = req.body; 
  ...
  const firestore = new FirestoreService("reportes");
 
  try {
    await firestore.PostDocument(idReportes, data);
    res.status(200).send("User registered.");
  } catch (err) {
    console.error("noSQL error:", err);-
    res.status(500).send("Error registering user.");
